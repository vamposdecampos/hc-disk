...
FS diskTypes[] =
{
	...
	{
		"Opus Discovery", FS_OPUS_DISCOVERY, 
	...
	{		
		...
		if (!asHex && IsBasic)
		{
			...
			else if (isSCR)
			{
				...
				string fnGIF = fn;				
				fnGIF += ".gif";														
				ConvertSCR2GIF(buf1, fnGIF.c_str());
		...
		{			
			...
			dword txtLen = f->GetDataAsText(buf1, wrap);
			if (txtLen > 0)
				buf1[txtLen - 1] = '\0';						
			TextViewer((char*)buf1);
		}
